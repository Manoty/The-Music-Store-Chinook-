...
st.subheader("🌐 Country vs Global Genre Comparison")

if {"revenue_month", "genre_name", "genre_revenue_country", "genre_revenue_global"}.issubset(df.columns):

    country_genre = country_df[country_df["genre_name"] == selected_genre].groupby("revenue_month", as_index=False)["genre_revenue_country"].sum()
    global_genre = df[df["genre_name"] == selected_genre].groupby("revenue_month", as_index=False)["genre_revenue_global"].sum()

    trend_merge = pd.merge(country_genre, global_genre, on="revenue_month", how="outer").fillna(0)

    if not trend_merge.empty:
        fig = go.Figure()
        fig.add_trace(go.Scatter(
            x=trend_merge["revenue_month"],
            y=trend_merge["genre_revenue_country"],
        ...
        st.plotly_chart(fig, use_container_width=True, key=f"country_vs_global_{selected_genre}_{selected_country}")
    else:
        st.info("No comparison data available.")
else:
    st.warning("Required columns for country/global genre comparison not found.")
